# Flow Farm Rust Ubuntu 编译脚本 (Docker临时容器)
...
REM 使用Docker临时容器编译 (--rm 自动删除，节省空间)
docker run --rm ^
    -v "%PROJECT_PATH%:/workspace" ^
    -w /workspace ^
    rust:1.78-slim ^
    bash -c "apt-get update -qq && apt-get install -y -qq pkg-config libssl-dev && echo '📋 安装完成系统依赖' && cargo --version && echo '🦀 开始编译 Rust 项目...' && cargo build --release && echo '✅ 编译完成！' && echo '📊 二进制文件信息:' && ls -lh /workspace/target/release/flow-farm-backend && echo '🎯 编译成功！'"
